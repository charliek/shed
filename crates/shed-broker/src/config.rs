...
",
    ...
    /// An on-disk config that still carries the retired `rc_hub:` section must
    /// LOAD, not error (plan 022 S6, `charliek/shed#328`). The reader is
    /// unknown-key-tolerant by construction and this is the test that keeps it
    /// that way: removing a knob must never break somebody's config file, and
    /// `configs/extensions.example.yaml` shipped this key for two releases.
    #[test]
    fn a_config_still_carrying_rc_hub_loads() {
        let dir = std::env::temp_dir().join(format!("rc-hub-residue-{}", std::process::id()));
        ...
        .unwrap();
        let cfg = HostAgentConfig::load(p.to_str().unwrap())
            .expect("a config carrying the retired rc_hub key still loads");
