...
// TestAcquireCreateLock_SerializesSameName verifies that two
// acquireCreateLock calls for the same shed name serialize — the second
// blocks until the first releases. This is the lock that closes the
// CreateShed / CopyRootfs TOCTOU race described in rootfs.go.
