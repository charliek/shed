...
// loadSnapshot reads a snapshot's metadata from disk.
...
// saveSnapshot writes snapshot metadata atomically (tempfile + rename).
...
// listSnapshotNames returns the names of all snapshots in snapshotsDir.
...
// Locks: takes the source shed's createLock (serializes against concurrent
// CreateSnapshot of the same source) and the snapshot's name lock (serializes
// against concurrent CreateSnapshot/DeleteSnapshot of the same name).
