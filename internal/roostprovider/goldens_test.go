...
// the vendored current-generation identify vector is asserted against roost's own
...
func TestSpokenProtocolMatchesVendoredVector(t *testing.T) {
	...
	readVectorJSON(t, "session.identify.response.v6.json", &vector)
	if vector.Result.SessionProtocol != SpokenProtocol {
		t.Errorf("SpokenProtocol = %d, the vendored v6 vector says %d",
