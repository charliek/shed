...
// The gated-input cells that used to close this file — the cursor input gate's
// accept/reject arms, the expired-working recovery, and the unanchored-widget
// residual — went with the gated lane in A6 (charliek/shed#322). POST /input answers
// 409 not_accepting for every kind now (hub_input_test.go); what survives here is the
// pane ANCHOR itself, which reconcile still uses to derive needs_approval.
