...
func TestClassifyRCProxyPath(t *testing.T) {
	cases := []struct {
		...
		// The cursor hook INGEST route was GUEST-INTERNAL and deliberately NOT on the
		// allowlist (plan 008 §3.5); A6 (charliek/shed#322) then removed it from the hub
		// entirely. Either way it must look like any other unknown path here — 404,
		// before any dial — in every method and query shape. Kept as a regression pin:
		// nothing outside the shed may inject a session's feed, whatever /v1/ingest/*
		// route a future lane introduces.
...
func TestRCProxy_ApprovalRoutes_RejectedBeforeDial(t *testing.T) {
	...
	cases := []struct {
		...
		// The cursor hook ingest route never was on the allowlist (it was guest-internal
		// even while it existed, and A6 — charliek/shed#322 — removed it from the hub
		// altogether). A caller reaching for it through the server proxy is still
		// rejected before the dial, query string and all.
