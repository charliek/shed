...
- **Refcount-protected prune.** `shed image prune` now walks `instances/*/metadata.json` and `snapshots/*/snapshot.json` and removes any blob whose digest has zero shed/snapshot references. Tags do **not** protect a digest (Docker model). `shed image rm <tag>` removes the tag only; the blob persists for `prune` to GC. `shed system prune --images` is wired through the same refcount path.
- **Metadata schema v2 (FC + VZ).** Instance metadata gains `lower_digest` + `lower_image_tag`, recorded at create time and inherited from snapshots when spawning. Snapshot metadata gains `lower_digest` so spawn-from-snapshot inherits the underlying-blob pin. Pre-v2 metadata loads now error with `metadata is from a pre-overlay version of shed; please run 'shed delete <name>' and recreate`.
- **CLI:** `shed image ls` (alias `list`), `shed image rm` (alias `delete`), `shed image inspect <tag-or-digest>`, `shed image tag <src> <new>`, `shed image pull <docker-ref> [-t <tag>]`. `shed image build` now writes into the blob store and advances a tag — no `.source` sidecar, no `LinkCachedImage` hop. `shed image ls` output gains DIGEST and IN USE columns.
- **API:** New `POST /api/images/tag`, `POST /api/images/pull`, `GET /api/images/inspect/{name}`. `ImagesResponse.images[]` gains `digest`, `tag`, and `in_use` fields. The shape of `DELETE /api/images/{name}` and `POST /api/images/prune` is unchanged but the *semantics* match the new model: delete = remove tag, prune = blob GC.
