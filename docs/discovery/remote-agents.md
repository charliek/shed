...
| **R2.6 — Ship `sx`** ⬅ **next** | The retirement's loose end: `sx` is now the ONLY machine RC path and it ships in no component (no brew, no apt, no release artifact), so a machine can only get it by building from source — mini3 has no Rust toolchain and needed a Docker build during R2.5's e2e. Wire `sx` into the release model as its own component (brew + apt, taking over the distribution slot machine-rc vacated), with the manifest-selected model's helper-only-tag shape so shipping it disturbs neither server nor desktop. **Dev-first**: cut a tag only when development needs `sx` on mini2/mini3; user-facing polish is not the bar | Unblocks every machine-target story, including R4's |
...
Sequencing from here: **R2.6 (ship sx) → R4 (machines in clients)**. R4 is
the payoff — mobile is design-critical and now has a stable machine hub to
talk to — and R2.6 is the small thing standing in front of it. **R3** is
independent of both and can slot in whenever the lane bet is due.
