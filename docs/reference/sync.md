...
```bash
# Copy a file into a shed (lands in the shed user's home)
scp localfile.txt shed-myproject:/home/shed/

# Copy a file out of a shed
scp shed-myproject:/home/shed/output.json ./
```
...
rsync -avz ./src/ shed-myproject:/home/shed/src/

# Sync with delete (mirror local to remote)
rsync -avz --delete ./src/ shed-myproject:/home/shed/src/
```
...
- **`shed sync`** — repeatable, config-driven syncing of dotfiles, certificates, and scripts (runs automatically on create)
- **`sftp`/`scp`** — ad-hoc file transfers
- **`rsync`** — efficient incremental syncing of large directories
