...
**Step 1.** Re-pull the configured images at the new release
version. This advances the on-disk tag to the new manifest digest;
the old manifest blob is now unreferenced by a tag (only by any
shed that's still booted off it).
...
**Step 3.** GC anything no longer reachable from a tag, a
running shed, or a snapshot pin. With the v0.5.8 fix, this is
safe to run any time; the configured tags survive.
