...
Every module *except* `test_mtls.py` is unaffected by `SHED_DEV_AUTH_MODE` —
they don't read it, and the dev server's token-mode invariants (`https_port`,
`auth.ssh`) are unchanged from before this knob existed.
