...
// stateDir returns the directory holding the agent's own PERSISTENT state —
// today just its client-credential store (see credstore.go).
//
// It is resolved separately from socketDir, and differs from it on Linux on
// purpose. socketDir prefers $XDG_RUNTIME_DIR, which is the correct home for a
// socket (per-boot, tmpfs, cleaned on logout) and exactly the wrong home for a
// private key: the agent would silently re-enroll over SSH on every reboot,
// and, worse, "it worked yesterday" would depend on whether the tmpfs had been
// cleared. State goes to the durable per-user data dir instead.
//
// SHED_HOST_AGENT_STATE_DIR overrides it, mirroring the
// SHED_HOST_AGENT_SOCKET_DIR escape hatch for tests and parallel dev agents.
func stateDir() string {
	if d := os.Getenv("SHED_HOST_AGENT_STATE_DIR"); d != "" {
		return d
	}
	if runtime.GOOS == "darwin" {
		return filepath.Join(userHomeDir(), "Library", "Application Support", "shed")
