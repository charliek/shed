#!/bin/bash
# Publish Firecracker Docker images to a container registry.
#
# Builds all Firecracker image variants and pushes them to the configured registry.
...
#
# Usage:
#   ./scripts/publish-fc-images.sh --version v1.0.0                # publish all variants
#   ./scripts/publish-fc-images.sh --version v1.0.0 --variant base # publish one variant
#   ./scripts/publish-fc-images.sh --version v1.0.0 --dry-run      # build only, don't push
#   ./scripts/publish-fc-images.sh --version v1.0.0 --registry ghcr.io/myorg  # custom registry
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Build and publish Firecracker Docker images to a container registry."
            echo ""
            echo "Options:"
...
# Build shed-agent binary for linux/amd64
build_agent() {
    echo ""
...
publish_variant() {
    ...
    local image_name="${REGISTRY}/shed-fc-${variant}"
...
# Main execution
echo "=== Publish Firecracker Docker Images ==="
...
if [ "$BUILD_ALL" = true ]; then
    for v in $KNOWN_VARIANTS; do
        if [ "$DRY_RUN" = true ]; then
            echo "  ${REGISTRY}/shed-fc-${v}:${VERSION} (local only)"
        else
            echo "  ${REGISTRY}/shed-fc-${v}:${VERSION}"
        fi
    done
else
    if [ "$DRY_RUN" = true ]; then
        echo "  ${REGISTRY}/shed-fc-${VARIANT}:${VERSION} (local only)"
    else
        echo "  ${REGISTRY}/shed-fc-${VARIANT}:${VERSION}"
    fi
fi
